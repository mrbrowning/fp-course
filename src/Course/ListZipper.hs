...
findLeft p (ListZipper xs a ys) =
    let (ls, rs) = break p xs
    in case rs of
        Nil -> IsNotZ
        (x :. rs') -> IsZ $ ListZipper rs' x (ls ++ (a :. ys))
...
findRight p (ListZipper xs a ys) =
    let (ls, rs) = break p ys
    in case rs of
        Nil -> IsNotZ
        (x :. rs') -> IsZ $ ListZipper (reverse ls ++ (a :. xs)) x rs'
...
moveLeftLoop (ListZipper Nil a xs) =
    let rs = reverse xs
        (last :. rs') = rs
    in ListZipper (rs' ++ (a :. Nil)) last Nil
...
moveRightLoop (ListZipper xs a Nil) =
    let rs = reverse xs
        (r :. rs') = rs
    in ListZipper Nil r (rs' ++ (a :. Nil))
...
dropLefts (ListZipper _ a xs) = ListZipper Nil a xs
...
moveLeftN n l@(ListZipper xs a ys)
    | n == 0 = IsZ l
    | n > 0 && n <= length xs =
        let (as, bs) = splitN n xs
            (a' :. as') = reverse as
        in IsZ $ ListZipper bs a' (as' ++ (a :. ys))
    | n < 0 && (negate n) <= length ys =
        let (as, (b :. bs)) = splitN (negate $ n + 1) ys
        in IsZ $ ListZipper ((reverse (a :. as)) ++ xs) b bs
    | otherwise = IsNotZ

splitN :: Int -> List a -> (List a, List a)
splitN n xs
    | n <= length xs = f n xs Nil
    | otherwise = error "List not long enough to split"
        where
            f 0 xs' ys' = (reverse ys', xs')
            f n' (x :. xs') ys' = f (n' - 1) xs' (x :. ys')
...
moveLeftN' n l@(ListZipper xs _ ys) = case moveLeftN n l of 
    IsNotZ -> Left $ length (if n >= 0 then xs else ys)
    IsZ l' -> Right l'
...
nth n l@(ListZipper xs _ _) = moveLeftN (length xs - n) l
...
index (ListZipper xs _ _) = length xs
...
-- prop> rights (end lz) == Nil
    ...
    let (b :. bs) = reverse (a :. ys)
    in ListZipper (bs ++ xs) b Nil
...
start (ListZipper xs a ys) =
    let (b :. bs) = reverse xs
    in ListZipper Nil b (bs ++ (a :. ys))
...
insertPushLeft x (ListZipper xs a ys) = ListZipper (a :. xs) x ys
...
insertPushRight x (ListZipper xs a ys) = ListZipper xs x (a :. ys)
...
instance Applicative ListZipper where
  ...
  (<*>) (ListZipper lfs f rfs) (ListZipper xs a ys) = ListZipper (zipWith id lfs xs) (f a) (zipWith id rfs ys)
...
instance Extend ListZipper where
  (<<=) f a = ListZipper (f' moveLeft a) (f a) (f' moveRight a)
      where
          f' g =
              let h = ((\x -> (x, x)) <$>) . toOptional . g
              in map f . unfoldr h
...
instance Traversable ListZipper where
  traverse f (ListZipper xs a ys) = ListZipper <$> (sequence $ map f xs) <*> (f a) <*> (sequence $ map f ys)
