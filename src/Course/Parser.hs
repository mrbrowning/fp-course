...
data ParseError = UnexpectedEof | ExpectedEof Input | UnexpectedChar Char | Failed
  deriving Eq
...
list a = ((:.) <$> a <*> list a) ||| valueParser Nil
...
satisfy p = f =<< character
    where
        f c = if p c then valueParser c else unexpectedCharParser c
...
-- True
natural :: Parser Int
natural = f <$> list1 digit
    where
        f = sum . repeatedly (*10) . reverse . map g
        g c =
            let (r :: Optional Int) = read $ (\x -> x :. Nil) c
            in case r of
                Full x -> x
                
repeatedly :: (a -> a) -> List a -> List a
repeatedly _ Nil = Nil
repeatedly f (x :. xs) = x :. g f xs
    where
        g _ Nil = Nil
        g f' (x' :. xs') = f' x' :. g (f . f') xs' 
...
personParser = Person <$> ageParser <* space <*> firstNameParser <* space <*> surnameParser <* space <*> smokerParser <* space <*> phoneParser
