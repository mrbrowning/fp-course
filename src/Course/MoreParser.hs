...
betweenCharTok l r p = between (charTok l) (charTok r) (p <* spaces)
...
hex = f =<< (replicateA 4 $ satisfy isHexDigit)
    where
        f cs =
            let ((Full x) :: Optional Int) = readHex cs
            in valueParser $ chr x
...
eof :: Parser ()
eof = P $ \s ->
    if isEmpty s then Result Nil () else ErrorResult $ ExpectedEof s
