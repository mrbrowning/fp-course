...
firstRepeat xs = eval (findM p xs) $ S.fromList []
    where
        p x = do
            seen <- get
            put $ S.insert x seen
            return $ S.member x seen
...
-- True
isHappy :: Int -> Bool
isHappy n = contains 1 $ firstRepeat sums
    where sums = map sum $ produce (digits . sum . map (P.^ 2)) $ digits n

digits :: Integral a => a -> List a
digits n = f Nil n
    where
        f xs 0 = xs
        f xs dividend = f ((rem dividend 10) :. xs) (div dividend 10)
