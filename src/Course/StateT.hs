...
distinct' xs = eval' (filtering p xs) $ S.fromList []
    where
        p :: (Ord a) => a -> State' (S.Set a) Bool 
        p n = do
            seen <- getT
            putT $ S.insert n seen
            return $ not $ S.member n seen
...
distinctF xs = evalT (filtering p xs) $ S.fromList []
    where
...
distinctG xs = runOptionalT $ evalT (filtering p xs) $ S.fromList []
    where
        p :: (Integral a, Show a) => a -> StateT (S.Set a) (OptionalT (Logger Chars)) Bool
        p n = StateT $ OptionalT . f
            where   
                f s
                    | n > 100 = log1 ("aborting > 100: " ++ lshow n) Empty
                    | n `rem` 2 == 0 = (log1 ("even number: " ++ lshow n)) . Full $
                        (not $ S.member n s, S.insert n s)
                    | otherwise = Logger Nil $ Full (not $ S.member n s, S.insert n s)

lshow :: (Show a) => a -> Chars
lshow = fromStandardList . show

fromStandardList :: [a] -> List a
fromStandardList [] = Nil
fromStandardList (x:xs) = (x :. Nil) ++ fromStandardList xs
